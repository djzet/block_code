...
class Environment {
    ...
    getVarType(name: string): 'number' | 'string' {
        if (this.vars[name] === undefined) {
            throw new Error(`Переменная ${name} не объявлена`);
        }
    ...
    setVar(name: string, value: number | string, expectedType: 'number' | 'string'): void {
        if (this.vars[name] === undefined) {
            throw new Error(`Переменная ${name} не объявлена`);
        }
    ...
    declareVar(name: string, type: 'number' | 'string'): void {
...
class StrDeclBlock extends BaseBlock {
    ...
    async runAction(env: Environment): Promise<void> {
        const val = this.element.querySelector('input')?.value || '';
        const names = val.split(',').map(s => s.trim()).filter(s => s);
